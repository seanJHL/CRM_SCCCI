...
import { EmailBody } from "@/components/crm/mobile/email-body";
      ...
      {classification && (
        <div className="m-card p-3.5">
          <button
            ...
            className="m-press flex w-full items-center justify-between gap-3"
            aria-expanded={classificationOpen}
          >
            <div className="min-w-0 text-left">
              <p className="text-[9px] font-bold uppercase tracking-[0.1em] text-[var(--m-text-3)]">Classification</p>
              <p className="mt-0.5 truncate text-[13px] font-semibold text-[var(--m-text)]">
                <span className="capitalize">{classification.category}</span>
                <span className="mx-1.5 text-[var(--m-text-3)]">·</span>
                <span className="capitalize font-medium text-[var(--m-text-2)]">{classification.priority}</span>
              </p>
            </div>
            <ChevronDown
              width={16}
              height={16}
              className={cn(
                "shrink-0 text-[var(--m-text-3)] transition-transform duration-150",
          ...
          {classificationOpen && (
            <div className="mt-3.5 space-y-3 border-t border-[var(--m-border)] pt-3.5">
                ...
                <ul className="mt-1.5 space-y-1">
                  {classification.importanceReasons.length ? (
                    classification.importanceReasons.map((reason) => (
                      <li key={reason} className="flex gap-1.5 text-[12px] text-[var(--m-text-2)]">
                        <Info width={12} height={12} className="mt-0.5 shrink-0" />
                        {reason}
                      </li>
                    ))
                  ) : (
                    <li className="text-[12px] text-[var(--m-text-2)]">No elevated importance signals found.</li>
                  )}
                </ul>
              </div>
            </div>
        ...
        {thread.messages.map((message) => (
          <article key={message.id} className="m-card overflow-hidden">
            <div className="flex items-center justify-between gap-2 border-b border-[var(--m-border)] bg-[var(--m-surface-2)] px-3.5 py-2.5">
              <span className="min-w-0 flex-1 truncate text-[13px] font-semibold text-[var(--m-text)]">
                {message.fromName || message.fromEmail}
              </span>
              <time className="shrink-0 text-[10px] text-[var(--m-text-3)]">{formatMessageDate(message.internalDate)}</time>
            </div>
            <EmailBody html={message.bodyHtml} fallbackText={message.bodyText || message.snippet} />
          </article>
        ))}
      </div>

      <div className="flex items-center justify-between gap-3 border-t border-[var(--m-border)] pt-3">
        <span className="text-[12px] font-semibold text-[var(--m-text-2)]">Status</span>
        <MobileFilterSelect
          label="Status"
